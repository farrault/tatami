...
public class TatamibotSpringTest extends CamelTestSupport {
    
    @Configuration
    @ComponentScan(basePackageClasses=Tatamibot.class)
    public static class MyConfig {
        @Bean
        public DomainRepository domainRepository() { return mock(DomainRepository.class); }
        
        @Bean
        public TatamibotConfigurationRepository tatamibotConfigurationRepository() { return  mock(TatamibotConfigurationRepository.class); }
        
        @Bean
        public UserServiceIface userService() { return mock(UserServiceIface.class);}
//        public UserService userService() { return mock(UserService.class);}
        
        @Bean
        public StatusUpdateServiceIface statusUpdateService() { return  mock(StatusUpdateServiceIface.class); }
        
        @Bean
        public MemoryIdempotentRepository idempotentRepository() { return new MemoryIdempotentRepository(); }
    }

    private static final Log log = LogFactory.getLog(TatamibotSpringTest.class);
...
//    private LastUpdateDateTatamibotConfigurationUpdater lastUpdateDateTatamibotConfigurationUpdater;
    ...
    public void setup() throws Exception {
        ...
        when(myConfig.userService().getUserByLogin("tatamibot@ippon.fr")).thenReturn(tatamibotUser);
        when(myConfig.tatamibotConfigurationRepository().findTatamibotConfigurationById(Mockito.anyString())).thenReturn(new TatamibotConfiguration());
    }
    ...
    public void testRssRouteOnly() throws Exception {
        ...
        verify(myConfig.statusUpdateService()).postStatusAsUser(msg1+" #BlogIppon", tatamibotUser);
        verify(myConfig.statusUpdateService()).postStatusAsUser(msg2+" #BlogIppon", tatamibotUser);
        verify(myConfig.statusUpdateService()).postStatusAsUser(msg3+" #BlogIppon", tatamibotUser);
        verifyNoMoreInteractions(myConfig.statusUpdateService());
        ...
        verify(myConfig.tatamibotConfigurationRepository(),times(3)).updateTatamibotConfiguration(argumentCaptor.capture());
        ...
        assertTrue(myConfig.idempotentRepository().contains("ippon.fr-"+msg1));
    }
...
//                    System.out.println("testing");
                    ...
                    int nbCalls = new MockUtil().getMockHandler(myConfig.statusUpdateService()).getInvocationContainer().getInvocations().size();
//                    System.out.println("tested");
    ...
    private void setupAndLaunchContext(TatamibotConfiguration configuration) throws Exception {
        ...
        when(myConfig.domainRepository().getAllDomains()).thenReturn(newHashSet(domain));
        when(myConfig.tatamibotConfigurationRepository().findTatamibotConfigurationsByDomain("ippon.fr")).thenReturn(newHashSet(configuration));
